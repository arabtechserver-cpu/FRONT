  ...
  const handleSaveReply = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (onOrderUpdated) onOrderUpdated({ ...order, status: nextStatus, code: replyCode });
      } else {
          ...
          <div style={{ padding: "14px 20px", background: "#ffffff", borderTop: "1px solid #f1f5f9" }}>
            <div style={{ display: "flex", gap: "10px", alignItems: "center" }}>
              <input
                ...
                readOnly
                style={{ flex: 1, padding: "8px 12px", border: "1px solid #cbd5e1", borderRadius: "6px", background: "#f8fafc" }}
              />
              <span style={{ fontSize: "0.8rem", color: "#64748b" }}>Read-only in live session</span>
            </div>
          </div>
        )}
