...
export default function ApiResellersTab({
  ...
  const [apiPaymentMode, setApiPaymentMode] = useState("prepaid");
  ...
  const openEditModal = async (customer) => {
    ...
    setApiPaymentMode(customer.api_payment_mode || "prepaid");
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/customer/admin/${editingCustomer.id}`, {
        ...
        body: JSON.stringify({
          ...
          api_payment_mode: apiPaymentMode,
          ...
          <div className="modal-content" onClick={e => e.stopPropagation()} style={{ maxWidth: "550px", background: "var(--surface-color, #111827)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: "16px", boxShadow: "0 25px 50px -12px rgba(0, 0, 0, 0.5)" }}>
            ...
            {activeTab === "settings" && (
              <form onSubmit={handleSaveApiSettings} className="modal-body" style={{ display: "flex", flexDirection: "column", gap: "15px" }}>
                ...
                <div className="form-group">
                  <label>نظام الدفع (Payment Mode):</label>
                  <select
                    className="form-input-premium"
                    value={apiPaymentMode}
                    onChange={(e) => setApiPaymentMode(e.target.value)}
                  >
                    <option value="prepaid">مسبق الدفع - يخصم من المحفظة (الوضع الافتراضي)</option>
                    <option value="postpaid">آجل - لا يخصم رصيد (يحتاج موافقة يدوية)</option>
                  </select>
                  <small style={{ color: "#94a3b8", marginTop: "4px", display: "block" }}>
                    في حالة "آجل"، لن يتم خصم رصيد من الموزع، وستصلك رسالة للموافقة على الطلب عبر تيليجرام أو لوحة التحكم.
                  </small>
                </div>
            ...
            {activeTab === "orders" && (
              <div className="modal-body" style={{ maxHeight: "400px", overflowY: "auto", padding: "10px" }}>
                {(() => {
                  ...
                  if (customerOrders.length === 0) return <p style={{ color: "#94a3b8", textAlign: "center", padding: "20px" }}>لا توجد طلبات API لهذا الموزع.</p>;
                  
                  return (
                    <div className="premium-table-wrapper" style={{ margin: 0, boxShadow: "none" }}>
                      <table className="premium-table" style={{ fontSize: "0.85rem" }}>
                        ...
                        <tbody>
                          {customerOrders.map(order => (
                            <tr key={order.id}>
                              <td style={{ fontWeight: "bold", color: "#38bdf8" }}>#{order.id}</td>
                              <td>{order.service_name}</td>
                              <td style={{ color: "#34d399", fontWeight: "bold" }}>{order.package_price} {order.baseCurrency || 'USD'}</td>
                              <td>
                                <span className={`premium-badge ${order.status === 'completed' ? 'premium-badge-approved' : order.status === 'pending' ? 'premium-badge-pending' : 'premium-badge-rejected'}`} style={{ padding: "4px 8px", fontSize: "0.75rem" }}>
                                  {order.status}
                                </span>
                              </td>
                              <td style={{ color: "#94a3b8" }}>{new Date(order.created_at).toLocaleString('ar-EG', { dateStyle: 'short', timeStyle: 'short' })}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
