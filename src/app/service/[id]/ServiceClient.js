...
const isStrictImeiField = (field) => {
  ...
  const combined = `${normalizedName} ${normalizedLabel}`.trim();

  if (!combined) return false;

  const mentionsImei = combined.includes("imei") || combined.includes("ecid");
  const mentionsLink = combined.includes("link") || combined.includes("url") || combined.includes("http") || combined.includes("screenshot");

  if (mentionsLink) return false;
  return mentionsImei;
};

const isPrimaryImeiInputField = (field) => {
  const normalizedName = String(field?.name || field?.id || "").trim().toLowerCase();
  const normalizedLabel = String(field?.label || "").trim().toLowerCase();
  const combined = `${normalizedName} ${normalizedLabel}`.trim();
  ...
  const mentionsLink = combined.includes("link") || combined.includes("url") || combined.includes("http") || combined.includes("screenshot") || combined.includes("hint");
  if (mentionsLink) return false;

  return (
    combined.includes("imei") ||
    combined.includes("ecid") ||
    combined.includes("serial number") ||
    /\bsn\b/.test(combined)
  );
};

const looksLikeInvalidNumericImei = (value) => {
...
export default function ServiceDetail({ params, initialService = null }) {
  ...
  const activeFields = useMemo(() => {
    ...
    if (isApiService) {
      // Prefer the selected package fields for provider services, but keep
    ...
    for (const f of rawFields) {
      if (!f) continue;
      const rawName = String(f.name || f.id || f.api_name || "").trim();
      const rawLabel = String(f.label || "").trim();
      const fieldId = rawName.toLowerCase();
      const fieldLabel = rawLabel.toLowerCase();

      if (!fieldId && !fieldLabel) continue;

      // Filter out internal provider fields that are not meant for customers (e.g. Order Code)
      if (
        fieldId.includes("order_code") ||
        fieldId.includes("ordercode") ||
        fieldId.includes("order code") ||
        fieldLabel.includes("order code") ||
        fieldLabel.includes("order_code") ||
        fieldLabel.includes("أدخل order code") ||
        fieldId === "ref_code" ||
        fieldId === "refcode" ||
        fieldId === "supplier_code" ||
        fieldId === "provider_code" ||
        fieldId === "txn_id" ||
      ...
      const isImeiOrSn = isPrimaryImeiInputField(f);

      // Preserve explicit requirement from provider/database definition; only default to false if unspecified
      if (f.required === undefined || f.required === null) {
        f.required = false;
      } else {
        f.required = f.required === true || f.required === 'on' || f.required === 1 || f.required === '1';
      }
      ...
      const baseKey = fieldId.replace(/^custom_/, '');
      const baseLabelKey = fieldLabel.replace(/^custom_/, '').replace(/^أدخل\s*/, '');

      if (seen.has(baseKey) || (baseLabelKey && seen.has(`lbl_${baseLabelKey}`))) {
        continue;
      }

      seen.add(baseKey);
      if (baseLabelKey) seen.add(`lbl_${baseLabelKey}`);
      uniqueFields.push(f);
    }
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/orders`, {
        ...
        body: JSON.stringify({
          ...
          player_id: (() => {
            if (formData.player_id) return String(formData.player_id).trim();
            const imeiField = activeFields.find(f => isPrimaryImeiInputField(f));
            if (imeiField && formData[imeiField.name]) return String(formData[imeiField.name]).trim();
            return String(formData[activeFields[0]?.name] || "").trim();
          })(),
  ...
  const isGsmService = service?.api_service_type && ['imei', 'server', 'remote'].includes(service.api_service_type);
  const displayLabel = isGsmService ? "سعر الوحدة هو:" : "سعر الـ 1000 وحدة هو:";
  const displayPrice = isGsmService ? (service?.price_per_thousand || 0) / 1000 : (service?.price_per_thousand || 0);
  ...
  const customQuantitySection = (
    <div>
      ...
      <p style={{ fontSize: "0.85rem", color: "var(--accent-color)", marginBottom: "12px", fontWeight: "bold" }}>
        {displayLabel} {baseCurrency === 'USD' || baseCurrency === 'USDT'
          ? `$ ${Number(displayPrice).toFixed(isGsmService ? 3 : 2)}`
          : (service?.category_currency === 'USD' || service?.category_currency === 'USDT'
            ? `$ ${Number(displayPrice).toFixed(isGsmService ? 3 : 2)} (ما يعادل ${Number(displayPrice * (exchangeRates?.["USD"] || 50)).toFixed(2)} ${baseCurrency})`
            : `${Number(displayPrice).toFixed(2)} ${baseCurrency}`)} (أقل كمية: {minQty}{maxQty > 0 ? `، أقصى كمية: ${maxQty}` : ''})
      </p>
        ...
        <div className="service-details-layout" style={{ gridTemplateColumns: "1fr" }}>
          <div className="glass-panel" style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
            ...
            <div style={{ marginTop: "24px", textAlign: "center" }}>
              ...
              >
                <span>{activeFields.length > 0 ? "المتابعة لإدخال البيانات" : t("continueToCheckout")} {isContinueEnabled && (
                  <span>({(() => {
                    const isUsd = service.category_currency === 'USD' || service.category_currency === 'USDT' || baseCurrency === 'USD' || baseCurrency === 'USDT';
                    let usdPrice = 0;
                    let egpPrice = 0;
